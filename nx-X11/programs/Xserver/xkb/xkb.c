    ...
    if (stuff->bellClass == KbdFeedbackClass) {
        ...
        if (stuff->pitch != 0) {
            if (stuff->pitch == -1)
                ...
                k->ctrl.bell_pitch = stuff->pitch;
        }
        if (stuff->duration != 0) {
            if (stuff->duration == -1)
                k->ctrl.bell_duration = defaultKeyboardControl.bell_duration;
            else
                k->ctrl.bell_duration = stuff->duration;
        }
    }
        ...
        if (stuff->pitch != 0) {
            if (stuff->pitch == -1)
                ...
                b->ctrl.pitch = stuff->pitch;
        }
        if (stuff->duration != 0) {
            if (stuff->duration == -1)
                b->ctrl.duration = defaultKeyboardControl.bell_duration;
            else
                b->ctrl.duration = stuff->duration;
        }
    }
    else {
...
ProcXkbLatchLockState(ClientPtr client)
{
    ...
    if ((status == Success) && stuff->affectModLatches)
        status =
            XkbLatchModifiers(dev, stuff->affectModLatches, stuff->modLatches);
...
ProcXkbSetControls(ClientPtr client)
{
    ...
    if (stuff->changeCtrls & XkbInternalModsMask) {
        ...
        new.internal.real_mods |=
            (stuff->affectInternalMods & stuff->internalMods);
        new.internal.vmods &= ~stuff->affectInternalVMods;
        new.internal.vmods |=
            (stuff->affectInternalVMods & stuff->internalVMods);
        new.internal.mask =
            new.internal.real_mods | XkbMaskForVMask(xkbi->desc,
                                                     new.internal.vmods);
    }
                ...
                xkbBehaviorWireDesc * wire, XkbChangesPtr changes)
{
    ...
    if (maxRG > (int) xkbi->nRadioGroups) {
        ...
        if (xkbi->radioGroups)
            xkbi->radioGroups =
                (XkbRadioGroupPtr) _XkbRealloc(xkbi->radioGroups, sz);
        else
            xkbi->radioGroups = (XkbRadioGroupPtr) _XkbCalloc(1, sz);
        if (xkbi->radioGroups) {
...
ProcXkbSetMap(ClientPtr client)
{
    ...
    if ((stuff->present & XkbKeyBehaviorsMask) &&
        (!CheckKeyBehaviors
         (xkb, stuff, (xkbBehaviorWireDesc **) & tmp, &error))) {
        client->errorValue = error;
        return BadValue;
    }
    ...
    if ((stuff->present & XkbVirtualModMapMask) &&
        (!CheckVirtualModMap
         (xkb, stuff, (xkbVModMapWireDesc **) & tmp, &error))) {
        client->errorValue = error;
        return BadValue;
    }
    ...
    if ((xkb->min_key_code != stuff->minKeyCode) ||
        ...
        status =
            XkbChangeKeycodeRange(xkb, stuff->minKeyCode, stuff->maxKeyCode,
                                  &change);
        if (status != Success)
            return status;
    ...
    if (stuff->present & XkbKeySymsMask) {
        tmp =
            SetKeySyms(client, xkb, stuff, (xkbSymMapWireDesc *) tmp, &change,
                       dev);
        if (!tmp)
            goto allocFailure;
    }
    ...
    if (stuff->present & XkbKeyBehaviorsMask) {
        tmp =
            SetKeyBehaviors(xkbi, stuff, (xkbBehaviorWireDesc *) tmp, &change);
        if (!tmp)
            goto allocFailure;
    }
    if (stuff->present & XkbVirtualModsMask)
    ...
    if (stuff->present & XkbVirtualModMapMask)
        tmp =
            SetVirtualModMap(xkbi, stuff, (xkbVModMapWireDesc *) tmp, &change);
    if (((tmp - ((char *) stuff)) / 4) != stuff->length) {
...
ProcXkbListComponents(ClientPtr client)
{
    ...
    rep = (xkbListComponentsReply) {
        ...
        .nKeymaps = list.nFound[_XkbListKeymaps],
    ...
    if (finfo.xkb == NULL)
        ...
        if (finfo.xkb->map == NULL)
            ...
            if (rep.reported & XkbGBN_ClientSymbolsMask) {
                ...
                mrep.firstKeySym = mrep.firstModMapKey =
                    finfo.xkb->min_key_code;
...
ProcXkbGetDeviceInfo(ClientPtr client)
{
    ...
    rep = (xkbGetDeviceInfoReply) {
        ...
        .length = nameLen / 4,
        .present = wanted,
        .supported = XkbXI_AllDeviceFeaturesMask & (~XkbXI_KeyboardsMask),
        .unsupported = XkbXI_KeyboardsMask,
        .nDeviceLedFBs = 0,
        .firstBtnWanted = 0,
        .nBtnsWanted = 0,
        .firstBtnRtrn = 0,
