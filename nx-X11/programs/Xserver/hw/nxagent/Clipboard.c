 ...
 * to send out a notification to the real X server _only_ if the
 * clientCutProperty needs to be updated.
 * returns 1 if the SelectionNotify event has been sent, 0 otherwise
 *
...
int nxagentSendNotify(xEvent *event)
{
  ...
  if (event->u.selectionNotify.property == clientCutProperty)
  {
    ...
    XSelectionEvent eventSelection = {
      ...
      .time = CurrentTime,   //FIXME: clients should never send CurrentTime - we are a client here!
      .property = event->u.selectionNotify.property,
    };

    /*
     * On rhe real server, the right CLIPBOARD atom is
     * XInternAtom(nxagentDisplay, "CLIPBOARD", 1), which is stored in
     * lastSelectionOwner[i].selection. For PRIMARY there's nothing to
     * map because that is identical on all X servers (defined in
     * Xatom.h).
     */
    ...
    XSendEventHelper(nxagentDisplay, eventSelection.requestor, False, 0L, (XEvent *) &eventSelection);
...
{
  #ifdef DEBUG
  fprintf(stderr, "%s: Got called, property [%d][%s] window [%p] (0x%x).\n", __func__, property, NameForAtom(property), (void *)pWin, WINDOWID(pWin));
  #endif
