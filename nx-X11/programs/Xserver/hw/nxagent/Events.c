...
void nxagentQueueKeyEvent(int type, unsigned int keycode, Bool update_last_event_time, Time use_time)
{
  ...
  #ifdef DEBUG
  fprintf(stderr, "%s: Got [%d] KeyboardEvents for Key event.\n", __func__, n);
  #endif
  for (int i = 0; i < n; i++)
      ...
      case ButtonPress:
      {
                    ...
                    X.xbutton.subwindow == None))
        {
         nxagentLastEventTime = GetTimeInMillis();
          ...
          if (nxagentOption(Rootless))
          {
            valuators[0] = X.xmotion.x_root;
            valuators[1] = X.xmotion.y_root;
          }
          else
          {
            valuators[0] = X.xmotion.x - nxagentOption(RootX);
            valuators[1] = X.xmotion.y - nxagentOption(RootY);
          }
        ...
        if (X.xcrossing.detail != NotifyInferior)
        {
          ...
          if (pScreen)
          {
            ...
            int n = GetPointerEvents(nxagentEvents, nxagentPointerDevice, MotionNotify,
                                     0, POINTER_ABSOLUTE, 0, 2, valuators);
            #ifdef DEBUG
            fprintf(stderr, "%s: Got [%d] PointerEvents for MotionNotify at EnterNotify.\n", __func__, n);
            #endif
            for (int i = 0; i < n; i++)
              mieqEnqueue(nxagentPointerDevice, (InternalEvent*)(nxagentEvents + i)->event);
#endif
